import torch
from model import SimpleCNN,CNN,deepCNN,residualCNN,EfficientNet
...
if __name__ == '__main__':
    ...
    #model = residualCNN(size=64,in_channels=8)
    #model = deepCNN(size=256,in_channels=16)
    model = EfficientNet(input_channels=16)
    model.apply(weights_init)


    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    class_weights = torch.tensor([1.0, 1.0]).to(device)  # 类别1的权重比类别0大
    ...
    num_epochs = 50 # 将epoch设置为10roc_auc_score
    ...
    for epoch in range(num_epochs):
        ...
        train_auc = roc_auc_score(all_train_labels, all_train_outputs)  # 适用于多分类
                # 混淆矩阵
        predicted_labels = (all_train_outputs > 0.5).astype(int)
        cm = confusion_matrix(all_train_labels, predicted_labels)
        ...
        with torch.no_grad():
            ...
            # 混淆矩阵
            predicted_labels = (all_test_outputs > 0.5).astype(int)
    ...
    plt.plot(range(5, num_epochs+1), test_losses[4:], label='Test Loss')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
